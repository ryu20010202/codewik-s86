...
<div class="edit">
  <div class="edit-x">
    <div class="edit-a">
      <form action="{{ route('rubyUpdate', ['id' => $item->id ]) }}" method="POST" class="edit-mein">
        @csrf
        <div class="edit-code">
    ...
    <div class="edit-b">
      <form action="{{ route('rubyDestroy', ['id' => $item->id ]) }}" method="POST">
        @csrf
        {{ method_field('delete') }}
    ...
    <div class="edit-c">
      <div class="edit-back-link">
        <a href="{{ route('rubyShow', ['id' => $item->id]) }}">詳細画面に戻る</a>
      </div>
    </div>
  </div>
</div>
