...
<div class="edit">
  <div class="edit-x">
    <div class="edit-a">
      <form action="{{ route('railsUpdate', ['id' => $item->id ]) }}" method="POST" class="edit-mein">
        @csrf
        <div class="edit-code">
          <label class="edit-code-title">要素</label>
    ...
    <div class="edit-b">
      <form action="{{ route('railsDestroy', ['id' => $item->id ]) }}" method="POST">
        @csrf
        {{ method_field('delete') }}
    ...
    <div class="edit-c">
      <div class="edit-back-link">
        <a href="{{ route('railsShow', ['id' => $item->id]) }}">詳細画面に戻る</a>
      </div>
    </div>
  </div>
</div>
