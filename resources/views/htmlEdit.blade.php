...
<div class="edit">
  <div class="edit-x">
    <div class="edit-a">
      <form action="{{ route('htmlUpdate', ['id' => $item->id ]) }}" method="POST" class="edit-mein">
        @csrf
        <div class="edit-code">
    ...
    <div class="edit-b">
      <form action="{{ route('htmlDestroy', ['id' => $item->id ]) }}" method="POST">
        @csrf
        {{ method_field('delete') }}
    ...
    <div class="edit-c">
      <div class="edit-back-link">
        <a href="{{ route('htmlShow', ['id' => $item->id]) }}">詳細画面に戻る</a>
      </div>
    </div>
  </div>
</div>
